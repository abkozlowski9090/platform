var https = require('https'),
	...
	pluginRepos = {
		...
		'wordpress-seo':'git://github.com/wp-plugins/wordpress-seo.git',
		'contact-form-7':'git://github.com/crowdfavorite-mirrors/wp-contact-form-7.git',
		'timber-library':'git://github.com/wp-plugins/timber-library.git',
		'Breadcrumb-NavXT':'git://github.com/mtekk/Breadcrumb-NavXT.git',
		'ewww-image-optimizer':'git://github.com/wp-plugins/ewww-image-optimizer.git',
		'updraftplus':'git://github.com/wp-plugins/updraftplus.git',
		'sucuri-scanner':'git://github.com/wp-plugins/sucuri-scanner.git',
		'thumbnail-upscale':'git://github.com/khromov/wp-thumbnail-upscale.git',
		'force-regenerate-thumbnails':'git://github.com/pedroelsner/force-regenerate-thumbnails.git'
	};
...
function getPluginVersion(callback, repo) {
	var latestVersion = '*';
	require('simple-git')().listRemote(['--tags', repo], function(err, tagsList) {
		if (err) return callback(err, latestVersion);
		tagList = ('' + tagsList).split('\n');
		tagList = tagList.filter(function(s){
			return s.indexOf("beta") <= 0;
		});
		tagList.pop();
		var lastTag = /([a-zA-z]+)?\d+\.\d+(\.\d+)?(-[A-Za-z]*)?/ig.exec(tagList.pop());
		if (lastTag !== null) {
			latestVersion = lastTag[0];
		}
		callback(null, latestVersion);
